...
parser.add_argument('--ori_model_dir', action='store', type=str,
                    default='model/pretrain/', help='directory to save model')
...
if __name__ == '__main__':
    ...
    ori_model_dir = args.ori_model_dir
    ...
    if ckpt is None:
        ckpt = tf.train.latest_checkpoint(checkpoint_dir=ori_model_dir)
        if ckpt:
            ### init all vars
            sess.run(tf.global_variables_initializer())

            #### filter trainable vars
            var_list = tf.trainable_variables(model.scope)
        
            ### restore part of model
            tmp_saver = tf.train.Saver(var_list=var_list)
            tmp_saver.restore(sess, ckpt)
            print('pretrain model {} has been loaded.'.format(ckpt))
        else:
            print('error -- no pretrain model found!!!')
            exit(-1)
    else:
